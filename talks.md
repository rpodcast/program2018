...
## _Leveraging the Cancer Targetome Evidence Framework to Visualize Uncertainty in Drug Target Interactions_

**Aurora S. Blucher**, PhD Candidate, Oregon Health & Science University

The precision cancer paradigm relies on our ability to match patients to the most appropriate targeted therapy based on their genetic variation. A key bottleneck in this area has been lack of accessibility and transparency for drug-target interaction information, especially with regards to supporting experimental evidence. Previously we have created the Cancer Targetome, which aggregates drug-target information from public resources and uniquely provides an evidence framework for the supporting information for each interaction. In this talk, we will highlight our work in R to visualize Cancer Targetome drug-target interaction data and accompanying supporting evidence. In particular, we will highlight our visualizations for target annotation across different binding assay types, which often provide discordant information. We will also demonstrate our work in R to visualize the relationship between pathway mutation status and drug response for the BeatAML acute myeloid leukemia cohort. Using probabilistic graphical modeling, we investigated both mutation impacted pathways in the cohort and drug-impacted pathways using Cancer Targetome data. We will demonstrate how our visualizations in R allow us to take multi-layered view on pathway modeling, including summarizing impacted pathways at the cohort level, investigating the spectrum of genes driving single pathway-drug associations, and the relationship between patient-specific pathway impact and drug response.
...
Qi Liu, Christopher Wen, Scott Pivirotto, Jin Jin, Matts Kagedal Genentech, Inc., South San Francisco, CA, USA
